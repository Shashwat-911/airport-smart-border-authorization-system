...
export default function VerificationPage() {
  ...
  const onScanComplete = async () => {
    console.log('Scan complete, fetching traveler data...');
    setCurrentStage(3);
    try {
      setError(null);
      ...
      console.log('Traveler data:', response.data);
      setTravelerData(response.data);
      setTravelerId(response.data.id || 'mock_traveler_123');
      
    } catch (err) {
      console.error('Verify API error:', err);
      // Fallback mock
      const mockData = {
        id: 'mock_traveler_123',
        name: 'John Doe',
        passport: 'AB1234567',
        nationality: 'USA',
        visa_status: 'VALID',
  ...
  const handleDecision = async (dec) => {
    console.log('Decision clicked:', dec, 'travelerId:', travelerId, 'user:', user);
    if (!travelerId) {
      console.error('No travelerId!');
      return;
    }
    ...
    try {
      ...
      console.log('Showing confirmation anyway for demo');
      setDecision(dec);
    } finally {
  ...
  useEffect(() => {
    console.log('VerificationPage mounted, user:', user);
  }, []);
